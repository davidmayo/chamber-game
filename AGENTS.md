...
- The front-left dish station (`Dish Station 1`) is an interactive seated console. Nearby `F` sits down; `F` or `Escape` stands up. Its chamber-style controls use `A`/`D` for dish azimuth and `W`/`S` for elevation, moving both dish reflectors in unison. Dish azimuth is a real compass bearing in the generated geographic basis: `0 degrees` is true north and `+90 degrees` is east; positive elevation is above the horizon.
