...
- The roofless second-floor hallway runs continuously past the DOC and server room and overlooks an empty first-floor high-bay box through large windows. There is intentionally no hallway door into the high bay. All areas coexist in `Main`; there are no scene portals, arrival markers, fades, or duplicate players. Keep the Play Mode test that physically walks the shared player across the chamber/hallway seam and verifies continuous floor support.
