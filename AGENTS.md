...
## First-floor Null Reference Laboratory

- The first floor contains the `Level 01 - Null Reference Laboratory` region directly below the chamber. It is generated by `GroundOpsSceneBuilder.NullLab.cs`. The user explicitly requested this location and its access stairs; it is completely independent of the high bay, which must remain sealed and inaccessible.
- `NullLabLayout` centralizes the stair dimensions, first-floor datum (`Y = -7.1`), and local area bounds. Coordinates are Ground Ops-local. Stair 01 enters through the far end of the chamber hallway at `(5.5, 0, 26.55)`. Its two 20-step flights run between `X = 4.2` and `X = -1.6`, with a half landing at `Y = -3.55`. The return flight stays beneath the containing-room floor. Preserve the walkable handrail clearance, landings, and headroom.
- Only the internal first-floor ceiling backing is split around the upper stair opening. The exterior building roof, foundation, and full-height facades remain complete independent slabs. Do not open a route into the high bay or alter the chamber's dimensions to accommodate the lab.
- The cable gallery leads to the supply isolator and the lab's side doorway. The null cell has a fixed glazed partition and a separate opening beside it. Ceilings remain opaque, collidable, and shadow-casting in runtime; `GeneratedCeilingSceneVisibility` hides them only in Scene View.
- `NullLaboratoryController` owns the optional cancellation experiment. F at the supply cabinet toggles bench power. The seated balance bench uses A/D phase, W/S amplitude, Shift fine control, and a Space hold to certify a residual no greater than 0.025. The fixed signal is amplitude 0.65 at +55 degrees; cancellation is amplitude 0.65 at -125 degrees. R after certification starts another test. Progress is session-only and independent of Signal Watch.
- The balance bench reuses `SimpleSeatedConsoleController`, including normal mouse look, wheel zoom, F/Escape exit, and restored standing FOV. That controller supports configurable prompts. Always locate the DSN console through its rack hierarchy when wiring Signal Watch; it is no longer the only SimpleSeatedConsoleController.
- The lab has its own named `Null Reference Laboratory` rendering layer (bit 6), local camera volumes, and reflection probe. Safety lighting stays on, while bench power controls the two task lights. Its residual tone is audible only while the player is in the new region. The inspection light and contextual field notebook use the same area bounds.
- Keep `NullLaboratoryTests` as a real Input System walkthrough of both stair flights in both directions, the gallery and lab doorways, supply, tuning, interrupted/accepted captures, zoom, notebook layout, and the independent high-bay wall. Its route markers describe a validation path; they never move the runtime player.
...
## Signal Archive / Afterglow

- `GroundOpsSceneBuilder.SignalArchive.cs` owns the first-floor archive beneath the DOC, inside the straight exterior facade. Its independent room shell spans Ground Ops local X `-3.95..5.5`, Z `-5.3..4.35`, floor Y `-7.1`, and ceiling underside Y `-0.4`. Preserve the open exterior beneath the curved DOC cantilever and the cavity below the second-floor slab.
- The amber passage extends the Null Lab cable gallery southward, with no high-bay connection. Its floor meets the old gallery at Z `12`. The Null Lab supply cabinet sits against the east gallery wall and retains its original interaction point `(4.5, -7.1, 13.2)`.
- `SignalArchiveController` owns three receiver supplies and three fictional light recordings. F toggles supplies; the shared seated console uses A/D to select and one Space press to play an automatic 18-second recording. Playback continues while exploring, freezes when paused, and tracks distinct recordings for the current session. These are imagined telemetry-inspired artworks, not physical measurement simulations.
