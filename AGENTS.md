...
- Chamber primitives are mirrored inside the builder's placement helpers, but plain generated `Transform` groups are not. Any non-primitive spatial marker such as a portal or arrival point must explicitly use `MirrorPosition`/`MirrorRotation`; otherwise its runtime behavior will be placed on the opposite wall from its visible geometry.
...
- The crude facility extension places a roofless second-floor hallway beyond the DOC/server-room `+X` wall. Interior windows look from the DOC into that hall, and large windows on the hall's opposite side overlook an empty first-floor high-bay box. There is intentionally no hallway door into the high bay. The DOC and server room are continuous parts of `GroundOps`; walking through the hallway's chamber double doors loads `Main`, and the matching containing-room double doors return to the named Ground Ops hallway arrival point. `FacilityScenePortal` owns these walk-through transitions, named arrivals, and the short black fade that hides scene replacement. Keep its bidirectional Play Mode test, including the post-arrival floor check; do not replace it with a test that calls scene loading directly.
